...
public class ModelGestureListener : MonoBehaviour, KinectGestures.GestureListenerInterface
{
                                  ...
                                  float progress, KinectInterop.JointType joint, Vector3 screenPos)
    {
        ...
        {
            //if(!GameObject.FindObjectOfType<GrabDropScript>().rightClick && canLeftClick)
            //{
                //GameObject.FindObjectOfType<GrabDropScript>().leftClick = true;
                //canLeftClick = false;
                //StartCoroutine(leftClickCooldown());
            //} 
        }
    }

    private IEnumerator leftClickCooldown()
    {
        yield return new WaitForSeconds(0.5f);
        GameObject.FindObjectOfType<GrabDropScript>().leftClick = false;
